...
void lz4CompressEngineRun(hls::stream<uintV_t>& inStream,
   ...
   // lz4_compress_engine<LOP>(inStream, lz4Out, lz4Out_eos, lz4OutSize, max_lit_limit, input_size, 0);
	lz4_compress_engine(inStream, lz4Out, lz4Out_eos, lz4OutSize, max_lit_limit, input_size, 0);
}*/

char  inputbuffer[6553600];
char outputbuffer[1000000];

int main(){

	FILE *fin = fopen("pages-1.img", "r");
	for(int i = 0; i < 6553600; i ++){
		char mytmp;
		fscanf(fin, "%c", &mytmp);
		inputbuffer[i] = mytmp;
	}
	lz4top(inputbuffer, outputbuffer);
    ...
    inputFile.open("D:/tixilab/lz4/pages-1.img", std::ofstream::binary | std::ofstream::in);
   ...
   // lz4CompressEngineRun(bytestr_in, bytestr_out, lz4Out_eos, lz4OutSize, max_lit_limit, input_size, 0);
    ...
    outputFile.open("D:/tixilab/lz4_new/hw/outputlz4.txt", std::fstream::binary | std::fstream::out);
